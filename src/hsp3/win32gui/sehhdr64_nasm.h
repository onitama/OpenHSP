...
; This header script is in the pulbic domain.
...
UWOP_PUSH_NONVOL        equ 0   ; Push a integer register
UWOP_ALLOC_LARGE        equ 1   ; Allocate a large buffer on the stack
UWOP_ALLOC_SMALL        equ 2   ; Allocate a small buffer on the stack (from 8 to 128 bytes)
UWOP_SET_FPREG          equ 3   ; Set up a stack frame
UWOP_SAVE_NONVOL        equ 4   ; Save a integer register on the stack by 'mov'
UWOP_SAVE_NONVOL_FAR    equ 5   ; Save a integer register on the stack with a long offset by 'mov'
UWOP_SAVE_XMM128        equ 8   ; Save a 28-bit XMM register on the stack by "mov"
UWOP_SAVE_XMM128_FAR    equ 9   ; Save a 28-bit XMM register on the stack with a long offset by 'mov'
...
struc UNWIND_INFO
    ...
    .Version:                       ; Version of the unwind data (3-bit)
    ...
    .ExceptionHandler:      resd 1  ; (Optional:    This exsists only when if UNW_FLAG_EHANDLER or UNW_FLAG_UHANDLER is set.)
    .ExceptionData          resd 1  ; (Optional:    You can obtain the address of this from DISPATCHER_CONTEXT.)
