...
void WebTask::onLoaded(unsigned handle, void* parent, void* file, unsigned length)
{
	auto webtask = static_cast<WebTask*>(parent);
	webtask->mode = CZHTTP_MODE_VARDATAEND;
	webtask->vardata = (char*)malloc( length + 1 );
	memcpy(webtask->vardata, file, length);
	webtask->vardata[length] = 0;
	webtask->size = length;
}

void WebTask::onError(unsigned handle, void* parent, int statuserror, const char* statusText)
{
	auto webtask = static_cast<WebTask*>(parent);
	char buffer[256];
...
void WebTask::onProgress(unsigned handle, void* parent, int progress, int total)
{
}
