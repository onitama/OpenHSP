...
*/		
	if ((a1>=0x30)&&(a1<=0x39)) {			// when 0-9 numerical
		...
		s3[a]=0;
		if ( wp != NULL ) {
			if ( *wp=='k' || *wp=='K' ) { fpflag=1;wp++; }
			if ( *wp=='f' || *wp=='F' ) { fpflag=2;wp++; }
			if ( *wp=='d' || *wp=='D' ) { fpflag=3;wp++; }
			if ( *wp=='e' || *wp=='E' ) { fpflag=4;wp++; }
			if ( *wp=='l' || *wp=='L' ) { fpflag=5;wp++; }
		}
...
char *CToken::ExpandToken( char *str, int *type, int ppmode )
{
	...
	if ((a1>=0x30)&&(a1<=0x39)) {			// when 0-9 numerical
		...
		if (( a1=='k' )||( a1=='f' )||( a1=='F' )||( a1=='d' )||( a1=='D' )||( a1=='l' )||( a1=='L' )) { s2[a++]=a1; vs++; }
		if (( a1=='e' )||( a1=='E' )) {
			s2[a++]=a1; vs++;
			a1=*vs;
			if (( a1=='-' )||( a1=='+' )) {
