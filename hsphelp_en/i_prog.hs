...
(Similar to C language and Java, "=" and "!" Can be described as "==" and "! =") If you want to change the program flow depending on the conditions,
^p
Example:
	a=10
	if a>10 : goto *over10
...
If iparam is 1, it is terminated by shutting down Windows (restarting or turning off the power). For termination processing when Windows shuts down, if an idle time (waiting time) is generated in the system by await, wait, stop instruction, etc., shut down.
...
When the break instruction is executed, even if the number of repetitions still remains, it is forced to exit the repetition and continues execution from the instruction following the loop instruction.
...
iparam: 0 (none)
^p
Even if you specify the post-error processing with the onerror instruction, when the necessary processing is completed, do not restart the application execution as it is, but end it with the end instruction as much as possible.
...
For beginners, it is recommended to repeat the description with the repeat ~ loop instruction or the if instruction.
%href
_continue
_break
wend
...
Please use it when you want to write something similar to C or Java.
For beginners, it is recommended to repeat the description with the repeat ~ loop instruction or the if instruction. The for macro internally generates a special program control instruction exgoto after macro expansion.
%href
_continue
_break
next
...
It is restarted from the instruction next to the loop without executing in the repeating block after the present.
%href
_continue
while
wend
