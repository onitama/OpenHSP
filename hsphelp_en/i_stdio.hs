...
^
If you specify a key code for p2 that can be detected even if you hold it down (the code in the table above, add each number if there are multiple), only that key will be detected as long as the button is pressed. It will be so.
^
This instruction looks very complicated, but it can be a very useful feature when writing scripts with keys.
For example, imagine a shooting game. Your aircraft must move in all directions while the buttons are pressed up, down, left and right. However, the next missile will not be fired until the missile launch button is pressed once.
...
The easing function is a convenient function that allows you to easily use advanced animations by mastering it, although it may be difficult to imagine the result until you get used to it.
