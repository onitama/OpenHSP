...
%inst
The counter for the repeat〜loop loop is assigned as an integer value.
...
^
※This system variable is defined as a macro within hspdef.as.
※It may not be obtainable depending on the device in HSP3Dish.
※For HTML5, an HTTPS connection is required. Please note that some browsers may not support this.
...
^
※This system variable is defined as a macro within hspdef.as.
※It may not be obtainable depending on the device.
※For HTML5, an HTTPS connection is required. Please note that some browsers may not support this.
