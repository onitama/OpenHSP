...
Items with the mode flag value of OBJ_LATE added (including those with a translucent 留 value set) are drawn after opaque objects, so the items are divided.
...
%index
setalpha
Object transparency (留 value) setting
%group
Extended Screen Control Commands
%prm
objid,value
...
%inst
Sets the transparency (留 value) of an object.
Sets the value specified by the value parameter to the object of the objid parameter.
The value specified by the value parameter is an integer value (留 value) from 0 to 255 indicating transparency.
0 is completely transparent, and 255 is opaque.
For objects with an 留 value other than 255 (semi-transparent), the drawing order is changed to the front.
